...
| `demo_fpv_counter`    | Saturating up-counter with a bound SVA checker for `rb fpv` (bmc-proves no-overflow, cover-reaches saturation); also the `rb mut` reference block | [`design/demo_abv/`](design/demo_abv/) ([detail](design/demo_abv/demo_fpv_counter.md)), [`fpv/demo_fpv_counter/`](fpv/demo_fpv_counter/) |
