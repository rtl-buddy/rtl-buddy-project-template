#!/usr/bin/env bash
# Regenerates the APB CSR RTL from spec/demo_alu_accel/demo_alu_accel_csr.rdl
# using PeakRDL. The generated SV files (demo_alu_accel_csr.sv,
# demo_alu_accel_csr_pkg.sv) are committed; CI runs this script and
...
RDL_FILE="${SCRIPT_DIR}/../../spec/demo_alu_accel/demo_alu_accel_csr.rdl"
OUT_DIR="${SCRIPT_DIR}"
...
uv run peakrdl regblock "${RDL_FILE}" \
  ...
  --cpuif apb4-flat \
  --default-reset rst_n \
  --module-name demo_alu_accel_csr \
  --package-name demo_alu_accel_csr_pkg

# Verilator-friendly lint waivers on the generated files
CSR_SV="${OUT_DIR}/demo_alu_accel_csr.sv"
CSR_PKG="${OUT_DIR}/demo_alu_accel_csr_pkg.sv"
