// demo_alu_accel_compute — Compute-domain (cclk) logic for the ALU accelerator.
//
...
//
// Drives one `demo_sandbox_alu` instance (instantiated via library include from
// design/demo_sandbox/demo_sandbox_alu.sv) and emits a 1-cycle result_valid pulse with
// {y, zf, cf, nf, vf} latched on the cycle the demo_sandbox_alu produces them.
//
// `busy` is high while a command is in flight or while the FIFO has
// entries waiting to be drained.

module demo_alu_accel_compute (
  input  logic       clk,
  input  logic       rst_n,
  ...
  // FIFO pop is one cycle ahead of demo_sandbox_alu launch (because fifo rd_data is
  ...
  // demo_sandbox_alu uses sync active-high rst; derive from the async-low rst_n.
  demo_sandbox_alu #(.W(8)) u_alu (
    .clk,
    .rst (~rst_n),
    .op  (op_q),
    .a   (a_q),
  ...
  // busy: any capture pending or demo_sandbox_alu in flight or fifo non-empty in stream mode
