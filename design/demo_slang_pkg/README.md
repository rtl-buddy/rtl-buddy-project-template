...
2. Once `rtl_buddy` ships [#90](https://github.com/rtl-buddy/rtl_buddy/pull/90) in a release (current state: open, draft / ready-for-review), bump the template's pinned `rtl_buddy` version. Until then, `frontend: "slang"` in synth.yaml is silently dropped by the released `rtl_buddy==4.0.0` — pyserde lenience — and the synth falls back to `read_verilog`, which fails on this design as documented above.
