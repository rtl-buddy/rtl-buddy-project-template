// demo_tiny_alu_subsys_compute — Compute-domain (cclk) logic for the ALU accelerator.
//
...
//
// Drives one `demo_tiny_alu` instance (instantiated via library include from
// design/demo_tiny_alu/demo_tiny_alu.sv) and emits a 1-cycle result_valid pulse with
// {y, zf, cf, nf, vf} latched on the cycle the demo_tiny_alu produces them.
//
...
module demo_tiny_alu_subsys_compute (
  ...
  // FIFO pop is one cycle ahead of demo_tiny_alu launch (because fifo rd_data is
  ...
  // demo_tiny_alu uses sync active-high rst; derive from the async-low rst_n.
  demo_tiny_alu #(.W(8)) u_alu (
    .clk,
    .rst (~rst_n),
    .op  (op_q),
    .a   (a_q),
  ...
  // busy: any capture pending or demo_tiny_alu in flight or fifo non-empty in stream mode
