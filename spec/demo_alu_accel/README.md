# `demo_alu_accel` — Multi-clock APB-mapped ALU Accelerator

The "system-level" demonstrator. Composes:

- [`design/demo_sandbox/alu.sv`](../../design/demo_sandbox/alu.sv) — the leaf compute
- [`design/apb/`](../../design/apb/) — APB SV interface
- [`design/common/`](../../design/common/) — `ip_cdc_sync`, `ip_cdc_handshake`, `ip_async_fifo`
- [`demo_alu_accel_csr.rdl`](demo_alu_accel_csr.rdl) — PeakRDL CSR description (compiled to SV by the design-side regen script)
- [`design/demo_alu_accel/`](../../design/demo_alu_accel/) — compute wrapper + multi-clock top

## Clock domains

| Clock     | Used by                                                        |
|-----------|----------------------------------------------------------------|
| `apb_clk` | APB bus, `demo_alu_accel_csr` (peakrdl-generated), FIFO write side |
| `cclk`    | `demo_alu_accel_compute` (drives the alu), FIFO read side           |
...
See [`specs.yaml`](specs.yaml). Verification: [`verif/demo_alu_accel/`](../../verif/demo_alu_accel/).
