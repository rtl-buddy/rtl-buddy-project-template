"""System-level golden reference for the demo_alu_accel block.

Wraps `sandbox_model.AluModel` with the simple semantics of the
accelerator: each accepted (op, a, b) eventually emits one
`AluResult` regardless of whether it came in via the CSR-direct path
or the streaming FIFO. Used by the demo_alu_accel build_report (post-run
...
from sandbox_model import AluModel, AluResult, OP_NAMES  # noqa: F401,E402
